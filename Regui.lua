...
D do if tick()-H>=0.3 and w:CanScrollUp()then task.wait() w:ScrollUp()w.Scrolled:Fire()end
...
D do if tick()-H>=0.3 and w:CanScrollDown() then task.wait() w:ScrollDown()w.Scrolled:Fire()
...
while F do if tick()-K>=0.3 and t(A) then task.wait() J()end wait()end end)x.
...
wrap(function()while G.Editable do task.wait() q.FastWait(0.5)if G.LastAnimTime~=J then
